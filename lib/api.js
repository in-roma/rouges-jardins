...
export async function getAllPodcasts(search = '') {
	const data = await fetchAPI(
		`query MyQuery($search: String = "") {
			posts(first: 1000, where: {categoryId: 8, search: $search}) {
			edges {
			  node {
				id
				title
...
export async function getMore(postId) {
	const data = await fetchAPI(
		...
		query PostsByDate($after: String) {
			posts(after: $after, first: 6, where: {categoryNotIn: [1,2,3,4,8,9,10]}) {
			  edges {
